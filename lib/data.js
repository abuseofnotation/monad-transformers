...
exports.maybe = {
  ...
  run (funk, mMaybeVal) {
    return this.outer.value((maybeVal) => {
      return {maybeVal: maybeVal.maybeVal === undefined ? maybeVal : funk(maybeVal.maybeVal)}
    }, mMaybeVal)
  },
...
exports.writer = {
  ...
  // ((val) => b, M([val, log])) => b
  run (funk, mWriterVal) {
    return this.outer.run((writerVal) => {
      return [ funk(writerVal[0]), writerVal[1] ]
    }, mWriterVal)
  },
  fold (fVal, fLog, writerVal) {
    fLog(writerVal[0])
    return f(writerVal[0])
  },
  // Custom functions
  tell (message, val) {
