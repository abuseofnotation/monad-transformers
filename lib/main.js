...
const idProto = {
  ...
  chain (funk) {
    return wrapIn(funk(this._value), idProto)
  }
}
/*
...
exports.make = function make_monad (outer, inner) {
  ...
  function create (val) {
    return wrapIn(val, stackProto)
  }

  // Define the prototype of the outer monad.
  const outerProto = {
    of (val) {
      return wrapIn(outer.of(val), outerProto)
    },
    ...
    chain (funk) {
      debugger
      return wrapIn(outer.chain(funk, this._value), outerProto)
    }
  }

  const innerProto = {
    of (val) {
      return wrapIn(inner.of(val), innerProto)
    },
    chain (funk) {
      return wrapIn(inner.chain(funk, this._value), innerProto)
    }
  }

  // Lifts a value from the outer type to a full stack
  const liftOuter = (val) => create(wrapIn(inner.lift(val), innerProto))
  const liftInner = (val) => create(inner.wrap(val, outerProto))
  ...
  const stackProto = {
    ...
    chain (funk) {
      ...
      return create(this._value.chain(funk))
    },
    // Add 'map' and 'of' functions
    of (value) {
      debugger
      const idValue = wrapIn(value, outerProto)
      return liftOuter(wrapIn(idValue, idProto))
    },
    map (funk) {
