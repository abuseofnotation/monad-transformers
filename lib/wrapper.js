...
const monad = {
  ...
  run (callback, environment) {
    ...
    const runFunctions = this.stackMembers.map((a)=>a.run)

    //Create a function that runs the monad stack with the specified callback
    const makeRunner = (callback, env) => helpers.asyncCompose([callback].concat(runFunctions), env)

    return makeRunner(callback, environment)(this._value)
  },
  toString () {
...
mmodule.exports = function make () {
  ...
  const promoteToMethod = (funk, monadDefinition) => function () {
    const args = Array.prototype.slice.call(arguments)
    return this.chain((val) => {
      return create(funk.apply(monadDefinition, args.concat([val])))
    })
  }

  // Promotes a function from a monad definition to a stack constructor
  const promoteToConstructor = (funk, monadDefinition) => function () {
    return create(funk.apply(monadDefinition, arguments))
  }

  // Augment the stack prototype with helper methods
  const stackProto = assign(baseStackProto, helpers.monadMapVals(promoteToMethod, stack.last))

  // The constructor function creates a new object and wraps it in the stack prototype
  const create = (val) => {
    return wrap(stackProto, val)
  }
  ...
  return assign.apply(null, [create].concat(stack.members.map(monad => helpers.monadMapVals(promoteToConstructor, monad))))
}
