  ...
  const baseStackProto = {
    ...
    value (callbacks) {
      ...
      const val = this.run((val)=>{
        ...
        const fold =  asyncCompose([foldId].concat(foldFunctionsBound))
        return fold(val)
      })
      return val
    },
    run (callback) {
      ...
      const run =  asyncCompose([callback].concat(runFunctions))
      return run (this._value)
    },
    toString () {
