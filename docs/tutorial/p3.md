...
That is precicely what we are going to do with our monad transformer.  The first part is to define it. We are going to start with the original implementation of the `Reader` monad transformer which is the following: 
...
The standars `Reader` helpers provide direct access to the environment to the functions we compose. Which is OK if the environment is immutable so our users can see it but cannot touch it but not OK in the current case. We would prefer to manipulate the environment from inside of the monad's implementation. 
