...
When doing IO we also have an environment on which we act on - the `process` object in nodeJS. so it make sense to use something as `Reader` as a base, only this time we are going to modify it  just a little bit, so it fits our needs exactly. 
