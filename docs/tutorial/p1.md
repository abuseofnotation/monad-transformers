...
- `suffix` is a function for concatenating strings, which we can use to construct a URL of a given resource. We are defining it using `curry` which means that calling it with just one argument will return a new function that  
