...
const idProto : monadDefinition <any> = {
  name: 'root',
...
const processProtoNew = (proto:monadDefinition<any>, outer:monadDefinitionProcessed<any>) => {
  const protoProcessed:monadDefinitionProcessed <any> = clone(proto)
...
const processStack = (baseStack) =>
  stateMap(baseStack, (item, state) => {
    const prevItemProcessed:monadDefinitionProcessed<any> = state.prevItemProcessed || idProto
    // Apply the processing function on each stack member
    const itemProcessed:monadDefinitionProcessed<any> = processProtoNew(item, prevItemProcessed)
    return [
        itemProcessed,
      {
...
export function createStack (monadStack:monadDefinition<any>) {
  // Generate errors
